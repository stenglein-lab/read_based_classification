...
workflow PREPROCESS_READS {
  ...
  */
  ...
  if (collapse_duplicates) {
    COLLAPSE_DUPLICATE_READS ( TRIM_READS.out.reads.filter{ it[1]*.getAt(0).size() > 0} ) 
  ...
  ch_multiqc_pre_files = ch_multiqc_pre_files.mix(TRIM_READS.out.log.collect{it[1]}.ifEmpty([]))
  if (collapse_duplicates) {
